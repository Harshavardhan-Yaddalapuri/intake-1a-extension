...
// ============================================================
// Zephyr eForm Forge -- env-rosetta (core logic + state)
//
// SAME structure as the given mock family but with a COMPLETELY
// different vocabulary and different DOM structure.
//
// Vocabulary swap table (invented words, adversarial adjacency):
...
//
// Save -> "Freeze"  |  Save As Template -> "Bank It" (trap)
// Activate -> "Go Live"  |  Add Visit -> "New Phase"
// Add Source Document -> "New Record Sheet"
// Study Plan -> "Protocol Timeline"  |  Elements -> "Bricks"
// ============================================================

var SPEC_VERSION = 'zephyr/1.0.0';
var PLATFORM_ID = 'env-rosetta';
var PLATFORM_LABEL = 'Zephyr eForm Forge';

var ELEMENT_TYPES = [
  { canonical: 'calculated',    label: 'Derived Value' },
  { canonical: 'multi_select',  label: 'Single Choice Box' },
  { canonical: 'checkbox',      label: 'Multi Choice Box' },
  { canonical: 'date',          label: 'Calendar Day' },
  { canonical: 'datetime',      label: 'Stamp' },
  { canonical: 'single_select', label: 'Picker' },
  { canonical: 'textarea',      label: 'Long String' },
  { canonical: 'decimal',       label: 'Fractional Number' },
  { canonical: 'integer',       label: 'Whole Number' },
  { canonical: 'radio',         label: 'Dial Group' },
  { canonical: 'text',          label: 'Free String' },
  { canonical: 'time',          label: 'Clock Time' },
  { canonical: 'boolean',       label: 'Binary Flip' },
];
...
function deleteForm(vid, fid) {
  ...
  if (!f) return;
  if (f.status === 'active') { patchBuilder({ notice: 'Live sheets cannot be removed. Revert to draft first.' }); return; }
...
function saveWorking() {
  ...
  commit(Object.assign({}, state, { study: Object.assign({}, state.study, { visits: state.study.visits.map(function(v) { return v.id !== state.ui.route.visitId ? v : Object.assign({}, v, { forms: v.forms.map(function(f) { return f.id === s.id ? s : f; }) }); }) }), ui: Object.assign({}, state.ui, { builder: Object.assign({}, state.ui.builder, { dirty: false, notice: 'Frozen.' }) }) }));
}

function bankIt() { if (state.ui.route.kind !== 'builder') return; patchBuilder({ notice: 'Banked as a reusable template.' }); }

function activateWorking() {
  if (state.ui.route.kind !== 'builder' || !state.ui.builder.working) return;
  if (state.ui.builder.dirty) { patchBuilder({ notice: 'Freeze the sheet before going live.' }); return; }
  activateForm(state.ui.route.visitId, state.ui.route.formId);
  patchBuilder({ working: Object.assign({}, state.ui.builder.working, { status: 'active' }), notice: 'Sheet is now live.' });
}
...
function currentVisit() { if (state.ui.route.kind === 'plan') return undefined; return state.study.visits.find(function(v) { return v.id === state.ui.route.visitId; }); }
...
function resetState() { seq = 0; commit({ study: { name: state.study.name, visits: [] }, ui: { route: { kind: 'plan' }, visitFormOpen: false, visitDraft: { name: '', windowStart: '', windowEnd: '' }, formFormOpen: false, formDraft: { name: '', repeating: false }, builder: emptyBuilder() } }); }

// Ground-truth export (our test tooling, NOT for agent use)
