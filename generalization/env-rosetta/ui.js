...
// ============================================================
// Zephyr eForm Forge -- env-rosetta (UI rendering)
// Top-toolbar layout (no left palette), palette as horizontal strip.
// ============================================================
...
function build() {
  ...
  app.append(el('div', { class: 'toolbar' }, [
    el('span', { class: 'toolbar-title' }, [text('Zephyr eForm Forge')]),
    el('button', { type: 'button', class: 'toolbar-btn', id: 'nav-phases' }, [text('Phases')]),
    el('button', { type: 'button', class: 'toolbar-btn', id: 'nav-sites' }, [text('Sites')]),
    el('button', { type: 'button', class: 'toolbar-btn', id: 'nav-data' }, [text('Data Entry')]),
    el('span', { class: 'toolbar-title', style: 'font-size:12px;color:#a8b4c8;' }, [text(state.study.name)]),
  ]));
...
function planScreen() {
  var nodes = [el('h2', {}, [text('Protocol Timeline')])];
  var table = el('table', { class: 'grid' });
  table.append(el('thead', {}, [el('tr', {}, [el('th', {}, [text('Phase')]), el('th', {}, [text('Window (days)')]), el('th', {}, [text('Record Sheets')]), el('th', {}, [text('')])])]));
  var body = el('tbody', {});
  ...
  if (state.study.visits.length === 0) body.append(el('tr', {}, [el('td', { colspan: '4', class: 'empty' }, [text('No phases defined.')])]));
  table.append(body);
  nodes.push(table);
  nodes.push(btn('add-visit', '+ New Phase', 'btn primary', function() { openVisitForm(); }));

  if (state.ui.visitFormOpen) {
    var card = el('div', { class: 'card' }, [el('h3', {}, [text('New Phase')])]);
    ...
    card.append(labelled('visit-name', 'Phase Name', name), labelled('visit-start', 'Window Start (day)', start), labelled('visit-end', 'Window End (day)', end),
      el('div', { class: 'actions' }, [btn('save-visit', 'Freeze Phase', 'btn primary', function() { saveVisit(); }), btn('cancel-visit', 'Cancel', 'btn', function() { cancelVisitForm(); })]));
    nodes.push(card);
  }
  return nodes;
}
...
function visitScreen() {
  ...
  if (!visit) return [el('p', {}, [text('Phase not found.')])];
  var nodes = [el('p', { class: 'breadcrumb' }, [text('Protocol Timeline / ' + visit.name)]),
    btn('back-to-plan', '<- Back', 'btn small', function() { navigate({ kind: 'plan' }); }),
    el('h2', {}, [text(visit.name + ' -- Record Sheets')])];
  var table = el('table', { class: 'grid' });
  table.append(el('thead', {}, [el('tr', {}, [el('th', {}, [text('Sheet')]), el('th', {}, [text('Version')]), el('th', {}, [text('Status')]), el('th', {}, [text('Type')]), el('th', {}, [text('Actions')])])]));
  var body = el('tbody', {});
  visit.forms.forEach(function(f) {
    var actions = el('td', {});
    if (f.status === 'draft') {
      actions.append(btn('edit-' + f.id, 'Modify', 'btn small', function() { openBuilder(visit.id, f.id); }));
      actions.append(btn('activate-' + f.id, 'Go Live', 'btn small', function() { activateForm(visit.id, f.id); }));
    } else {
    ...
    body.append(el('tr', {}, [el('td', {}, [text(f.name)]), el('td', {}, [text('v' + f.version)]), el('td', {}, [el('span', { class: 'chip ' + f.status }, [text(f.status === 'draft' ? 'Draft' : 'Live')])]), el('td', {}, [text(f.repeating ? 'Repeating log' : 'Standard')]), actions]));
  });
  if (visit.forms.length === 0) body.append(el('tr', {}, [el('td', { colspan: '5', class: 'empty' }, [text('No record sheets.')])]));
  table.append(body);
  nodes.push(table);
  nodes.push(btn('new-form', '+ New Record Sheet', 'btn primary', function() { openFormForm(); }));

  if (state.ui.formFormOpen) {
    var card = el('div', { class: 'card' }, [el('h3', {}, [text('New Record Sheet')])]);
    var name = el('input', { type: 'text' });
    ...
    card.append(labelled('form-name', 'Sheet Name', name), el('div', { class: 'row checkbox' }, [rep, el('label', { for: 'form-repeating' }, [text('Repeating log (many records per phase)')])]),
...
function builderScreen() {
  ...
  if (!visit || !working) return el('p', {}, [text('Sheet not found.')]);
  var wrap = el('div', {});
  var back = btn('builder-back', '<- ' + visit.name, 'btn small', function() { navigate({ kind: 'visit', visitId: visit.id }); });
  var save = btn('builder-freeze', 'Freeze', 'btn primary', function() { saveWorking(); });
  var bank = btn('builder-bank', 'Bank It', 'btn primary', function() { bankIt(); });
  var preview = btn('builder-preview', 'Preview', 'btn', function() { openPreview(); });
  var activate = btn('builder-activate', 'Go Live', 'btn primary', function() { activateWorking(); });
  var statusBits = ['v' + working.version, working.status === 'draft' ? 'Draft' : 'Live'];
  if (state.ui.builder.dirty) statusBits.push('Unfrozen changes');
  wrap.append(el('div', { class: 'builder-bar' }, [
    ...
    el('div', { class: 'builder-bar-right' }, [preview, bank, save, activate]),
  ]));
  if (state.ui.builder.notice) wrap.append(el('div', { class: 'toast' }, [text(state.ui.builder.notice)]));

  var palette = el('div', { class: 'palette-strip' }, [el('span', { style: 'font-weight:600;font-size:12px;color:#475569;align-self:center;' }, [text('Bricks:')])]);
  ...
  ELEMENT_TYPES.forEach(function(def) {
    ...
    var tile = el('button', { type: 'button', class: 'palette-tile', id: 'brick-' + def.canonical }, [text(def.label)]);
    tile.addEventListener('click', function() { addElement(def.canonical); });
    palette.append(tile);
  });
  var filter = el('input', { type: 'text', placeholder: 'Filter...', style: 'width:120px;' });
...
function canvasColumn(working) {
  ...
  if (!page || page.elements.length === 0) {
    surface.append(el('p', { class: 'empty' }, [text('Click a brick above to add it to this page.')]));
  } else {
...
function inertControl(element) {
  ...
  switch (element.type) {
    ...
    case 'date': h.append(el('input', { type: 'text', 'aria-label': element.label, placeholder: 'DD-MMM-YYYY', class: 'narrow' })); break;
    case 'time': h.append(el('input', { type: 'text', 'aria-label': element.label, placeholder: 'HH:MM', class: 'narrow' })); break;
    case 'datetime': h.append(el('input', { type: 'text', 'aria-label': element.label, placeholder: 'DD-MMM-YYYY HH:MM', class: 'narrow' })); break;
    case 'boolean': h.append(el('button', { type: 'button', class: 'btn small' }, [text('Yes')]), el('button', { type: 'button', class: 'btn small' }, [text('No')])); break;
...
function optionsColumn() {
  var col = el('aside', { class: 'options-panel' }, [el('h3', {}, [text('Properties')])]);
  var element = selectedElement();
  if (!element) { col.append(el('p', { class: 'empty' }, [text('Select an element on the canvas to edit its properties.')])); return col; }
  ...
  col.append(labelled('opt-type', 'Element Type', type));
  ...
  if (def.hasRange) {
    var fs = el('fieldset', {}, [el('legend', {}, [text('Range Check')])]);
    ...
    fs.append(labelled('opt-min', 'Minimum', min), labelled('opt-max', 'Maximum', max), labelled('opt-units', 'Units', units));
  ...
  if (def.hasFormula) {
    ...
    col.append(labelled('opt-formula', 'Formula', f));
  }

  if (def.hasOptions) {
    var fs2 = el('fieldset', { class: 'values' }, [el('legend', {}, [text('Values')])]);
    ...
    fs2.append(btn('val-add', '+ Add Value', 'btn small', function() { addValue(); }));
    ...
    fs2.append(labelled('val-paste', 'Paste Values (replaces list)', paste));
    fs2.append(btn('val-paste-apply', 'Apply Pasted Values', 'btn small', function() { applyPasteValues(); }));
    col.append(fs2);
  }

  // Visibility / skip logic
  var fs3 = el('fieldset', {}, [el('legend', {}, [text('Element Visibility')])]);
  var mode = el('select', {});
  ...
  if (element.visibility.mode === 'when') {
    ...
    fs3.append(labelled('opt-vis-when', 'When Element', when), labelled('opt-vis-val', 'Equals Value', eq));
  }
  col.append(fs3);
  col.append(btn('opt-delete', 'Delete Element', 'btn danger', function() { deleteSelectedElement(); }));
  return col;
}
