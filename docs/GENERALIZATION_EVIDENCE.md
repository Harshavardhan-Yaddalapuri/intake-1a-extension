...
Best live overalls below. Runs are **flaky** (SW keep-alive / visit-open gates); v8 regressed vs v7 on swapped-controls. Details in Live E2E v7 / v8 sections.

| Env | Best live overall |
|---|---|
| env-swapped-controls | **19.4%** (v7, 207/1067); **v8 regressed to 0.37%** (4/1067) |
| env-rosetta | **3.37%** (v5–v8 best, 36/1067) |
| env-wizard | **0.37%** (v6/v7, 4/1067) |
| env-hostile-a11y | **0%** (v7, 0/1067) |

Harness perfect-seed ceiling remains **100%** on all four. Do **not** treat these as ~70% never-seen claims.
...
## Live E2E v8 — after probe residue 3320b18 + parked-review UX 119f71b

Date: 2026-09-10 (computerUse), extension removed and freshly loaded unpacked from `/workspace/intake-1a/extension`; IR `takehome/data/abc-101-study.ir.json`. Fresh runs were started without pausing; no gate was approved, changed, or skipped. Runs stopped only after the untouched blocker condition was observed; the env-rosetta attempt also hit an extension-worker restart before it could resume.

| Env | Ground-truth dump | Score | Demographics labels / outcome |
|---|---|---:|---|
| `env-swapped-controls` (`:4094`) | `generalization-runs/env-swapped-controls-after-v8.json` | **0.37%** (4/1067) | **No Demographics fields** in the export (labels: none). Run paused at untouched `Screening` gate: `Not built — missing from the study`; `I could not open this visit, so nothing under it was built.`; `Nothing here has been built yet, and the build will not go past this until you answer.` After >2 minutes with no field progress, it was treated as the true blocker. |
| `env-rosetta` (`:4091`) | `generalization-runs/env-rosetta-after-v8.json` | **3.37%** (36/1067) | Demographics labels: `Derived Value`, `Single Choice Box`, `Multi Choice Box`, `Calendar Day`, `Stamp`, `Picker`, `Long String`, `Fractional Number`, `Whole Number`, `Dial Group`, `Free String`, `Clock Time`, `Binary Flip`, `Subject Initials`, `Date of Birth`, `Sex at Birth`, `Race`, `Ethnicity`, `Height`, `Weight`, `Body Mass Index`. Fresh start hit `Build agent stopped (extension worker restarted)` during pre-flight; no gate action was taken. Export remained at the observed Demographics state. |

Neither priority environment exceeded 15%, so `env-wizard` (`:4092`) and `env-hostile-a11y` (`:4093`) were not attempted under the conditional rule. Scorecards: `generalization-runs/env-{swapped-controls,rosetta}-after-v8-score.json`.
